...
export default function Home() {
  ...
  // YENİ: Süresi dolmak üzere olanlar (FOMO Etkisi)
  const [sonSansKampanyalar, setSonSansKampanyalar] = useState<any[]>([]); 
  ...
  useEffect(() => {
    ...
    const veriGetir = async () => {
      try {
        ...
        setUcretsizKampanyalar(karistir(ucretsizData).slice(0, 6)); 
        setEnYeniKampanyalar(karistir(yeniKData).slice(0, 6));     
        
        setTumAktifKampanyalar(tumAktifData);
        setKampanyaTurleri(tData);

        // YENİ: Son Şans Kampanyaları (Bitişine 3 günden az kalanlar)
        const ucGunSonra = new Date();
        ucGunSonra.setDate(ucGunSonra.getDate() + 3);
        const sonSans = tumAktifData.filter(k => 
          k.bitis_date && 
          new Date(k.bitis_date).getTime() <= ucGunSonra.getTime() && 
          new Date(k.bitis_date).getTime() > new Date().getTime()
        ).slice(0, 4);
        setSonSansKampanyalar(sonSans);
