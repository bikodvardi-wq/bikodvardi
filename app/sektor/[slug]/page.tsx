...
export default function SektorDetay({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [toplamAktifKod, setToplamAktifKod] = useState(0);
  ...
  useEffect(() => {
    ...
    const veriGetir = async () => {
      ...
      // Aktif kampanyaları çek (süresi dolmamış veya süresiz)
      ...
      const { data: mData } = await supabase
        ...
        .or(`sektor_id.eq.${sData.id},ek_sektor_idler.cs.{${sData.id}}`);
      ...
      const markalarliData = mData.map(marka => {
        ...
        const aktifKodSayisi = markaKampanyalari.length; // sadece aktif olanlar
        return { ...marka, kampanyaSayisi: aktifKodSayisi };
      }).sort((a, b) => b.kampanyaSayisi - a.kampanyaSayisi);

      // Toplam aktif kod sayısı (benzersiz)
      const benzersizAktifKodlar = new Set();
      kData.forEach(k => {
        if (String(k.gecerli_sektor_id) === String(sData.id) || 
            mData.some(m => String(k.fayd_marka) === String(m.id))) {
          benzersizAktifKodlar.add(k.id);
        }
      });

      setMarkalar(markalarliData);
      setToplamAktifKod(benzersizAktifKodlar.size);
      setYukleniyor(false);
    };
    veriGetir();
  }, [slug]);
